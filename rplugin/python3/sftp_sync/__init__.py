...
class SftpSync(object):
    def __init__(self, nvim):
        ...
        vars = self.nvim.vars
        self.enabled = True
        self.log_file = vars.get(
            "sftp_sync_log_file", "{}/{}".format(gettempdir(), "sftp_sync.log")
        )
    ...
    @pynvim.command("SftpRecv", nargs="?", complete="file")
    def sftp_recv(self, args):
        if not self.enabled:
            return
        try:
            file = self.nvim.funcs.fnamemodify(args[0], ":p")
    ...
    @pynvim.command(
        "SftpSelectServer", nargs=1, complete="customlist,_sftp_complete_server"
    )
